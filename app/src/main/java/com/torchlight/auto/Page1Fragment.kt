...
class Page1Fragment : Fragment() {
    ...
    override fun onCreateView(inf: LayoutInflater, c: ViewGroup?, s: Bundle?): View? {
        ...
        row2.addView(btn("放大") {
            cropL -= 0.03f; cropT -= 0.03f; cropR += 0.03f; cropB += 0.03f
            clamp(); showArea()
        })
        row2.addView(btn("缩小") {
            cropL += 0.03f; cropT += 0.03f; cropR -= 0.03f; cropB -= 0.03f
    ...
    private fun clamp() {
        cropL = cropL.coerceIn(0f, 0.9f)
        cropT = cropT.coerceIn(0f, 0.9f)
        cropR = cropR.coerceIn(0.1f, 1f)
        cropB = cropB.coerceIn(0.1f, 1f)
